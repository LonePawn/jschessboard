...
return {
  ...
  _drawPiece: function(glyph, color, squareName){
    ...
    b_context.strokeText(glyph, x_pos, y_pos);
  },
 ...
 setPieces: function(){
  ...
  showMovesFor: function(origin_square_name){
    ...
    defaultMoves = piece.destinationSquares(origin_square_name);
    // The board has the requisite knowledge to limit available movement squares based upon whether or not same-color pieces are placed there.
    availableMoves = defaultMoves.filter(function(sq_name){ 
                                           sq = self.squares.find(function(sq){ return sq.name == sq_name }); 
                                           return sq.piece == undefined || sq.piece.color != piece.color;   
    });
    // It should also be able to tell when a square is unreachable because there is a piece between it and it's destination
    availableMoves.forEach(function(square_name){
      self._strokeSquare(square_name, 'green');
    });
  },

  _strokeSquare: function(square_name, stroke_color){
  ...
  _centerOfSquare: function(squareName){
  file         = squareName[0];
 ...
 _rankByY: function(y_coord){
    self = this;
    switch(true){
      case(y_coord <= self.square_width()):
        return 8;
        break;
      case(((y_coord > self.square_width()) && y_coord <= (self.square_width() * 2))):
        return 7;
        break;
      case(((y_coord > self.square_width() * 2) && y_coord <= (self.square_width() * 3))):
        return 6;
        break;
      case(((y_coord > self.square_width() * 3) && y_coord <= (self.square_width() * 4))):
        return 5;
        break;
      case(((y_coord > self.square_width() * 4) && y_coord <= (self.square_width() * 5))):
        return 4;
        break;
      case(((y_coord > self.square_width() * 5) && y_coord <= (self.square_width() * 6))):
        return 3;
        break;
      case(((y_coord > self.square_width() * 6) && y_coord <= (self.square_width() * 7))):
        return 2;
        break;
      case(((y_coord > self.square_width() * 7) && y_coord <= (self.square_width() * 8))):
        return 1;
        break;
  }
 },

  _fileByX: function(x_coord){
    switch(true){
      case(x_coord <= this.square_width()):
        return 'a';
        break;
      case(((x_coord > this.square_width()) && x_coord <= (this.square_width() * 2))):
        return 'b';
        break;
      case(((x_coord > this.square_width() * 2) && x_coord <= (this.square_width() * 3))):
        return 'c';
        break;
      case(((x_coord > this.square_width() * 3) && x_coord <= (this.square_width() * 4))):
        return 'd';
        break;
      case(((x_coord > this.square_width() * 4) && x_coord <= (this.square_width() * 5))):
        return 'e';
        break;
      case(((x_coord > this.square_width() * 5) && x_coord <= (this.square_width() * 6))):
        return 'f';
        break;
      case(((x_coord > this.square_width() * 6) && x_coord <= (this.square_width() * 7))):
        return 'g';
        break;
      case(((x_coord > this.square_width() * 7) && x_coord <= (this.square_width() * 8))):
        return 'h';
        break;
  }
